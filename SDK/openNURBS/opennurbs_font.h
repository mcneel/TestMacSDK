...
public:
  ...
  */
  ...
  /*
  Description:
    Get a mesh of a sorted outline.
  Parameters:
    tolerance - [in]
      tolerance passed to ON_OutlineFigure::GetPolyline(tolerance,...)
      to create mesh boundary.
    scale - [in]
      If scale > 0.0, then all points are scaled this factor with (0,0) 
      as the fixed point. Otherwise all points are returned in font design units.
      ON_Font::ScaleFromTextHeight() is a good tool to get this value.
    mesh_points - [out]
      Mesh points are appended to mesh_points[].
    mesh_triangles - [out]
      Mesh triangles are appended to mesh_triangles[]
    mesh_boundary_area - [out]
      If not nullptr, the area of the polyline boundary is returned here.
    mesh_triangle_area - [out]
      If not nullptr, the area of the mesh is returned here.
      When mesh_boundary_area and mesh_triangle_area are different, 
      it is an indication that the boundary contains selft intersections 
      or incorrectly oriented figures and additional cleaning or validation
      of the outline may be needed to get desirable results.
    mesh_error_counter - [out]
      If not nullptr, warning and error counts are returned here. 
      When there are warnings or errors and a value > 0 is returned, 
      then a mesh is still returned but you may wish to perform 
      additional validation before using it.
  */
  unsigned int GetOutlineMesh(
    double tolerance,
    double scale,
