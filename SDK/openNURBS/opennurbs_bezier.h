...
public:
  ...
  */
  ...
  Parameters:
    ...
    sub_domain - [in]
      If not nullptr, the search is confined to the intersection
  ...
  */
  ...
  Parameters:
    ...
    this_seed_t - [in] this curve seed parameter
    other_seed_t - [in] other curve seed parameter
    this_t - [out] this curve parameter
    other_t - [out] other curve parameter
...
public:
  ...
  Returns:
    this + v
  Remarks:
    Ignores coordinates after the first three.
  */
  ...
  */
  ...
  Returns:
    this + B
  Remarks:
    m_dim and B.m_dim must be equal.
  */
  ...
  */
  ...
  Returns:
    this - crv
  Remarks:
    this and crv must have the same dimension.
  */
...
public:
  ...
  Remarks:
    A.m_dim must be 3.
  */
	static ON_BezierCurve CrossProduct( 
    const ON_BezierCurve& A,
    ON_3dVector V
    );
...
public:
  /*
  Returns:
    A o B (scalar dot product)
  Remarks:
    A.m_dim must be 3.
  */	
  static ON_BezierCurve DotProduct( 
    const ON_BezierCurve& A,
    ON_3dVector B
    );
  ...
  Returns:
    A o B (scalar dot product)
  Remarks:
    A.m_dim and B's dimension must be equal
  */
  static ON_BezierCurve DotProduct(
    const ON_BezierCurve& A,
    const double* B
  ...
  Returns:
    A o B (scalar dot product)
  Remarks:
    A.m_dim and B.m_dim must be qeual
  */		
  static ON_BezierCurve DotProduct(
    const ON_BezierCurve& A,
...
public:
  ...
  */
  static ON_BezierCurve Derivative(
    const ON_BezierCurve& A
    );

 /*
 Description: 
	 TL_Derivative( const ON_BezierCurve& Crv, ON_BezierCurve& Cdot)
	 Computes the bezier representing the  derivative of Crv .
	 Returns true if the result is the derivative.
 Details:
	 For non-rational Crv the order of the result is 2*Crv.Order()-1

 Remarks
	 For optimal performance use this version with derivativeA 
   allocated to the proper size, instead of using ON_BezierCurve::Derivative.  
   This will avoid allocating and copying of temporary objects. 
 Parameters:
   A - [in]
     A.Order() must be >= 2.
	
	 derivativeA - [out]
    For a non-rational A with degree > 1, the result is non-rational of degree A.Degree()-1.
    For non-rational A with degree 1, the result is degree 1.
    For rational A, the degee of derivativeA is 2*A.Degree() (quotient rule).
  Returns:                     
  ...
  */
  ...
  /*
	 Description:
		 Find all the zeros of the first coordinate of a bezier in the domain [0,1].  
     Roots are reported only once regardless of multiplicity.
	 Parameters
	  root  [out]
      An array of degree many doubles. Roots are returned here
			in increasing order
	  tol  [in]
      tolerance >0 used for detecting roots of even multiplicity
	
	 Details:
	    All roots are computed to full machine precision.  
			As an example of how tol is used
					(x-.123)^2 + tol/2 will have a root at .123
			where as
					(x-.123)^2 + 2*tol has no roots
...
public:
  ...
  */
  ...
  Parameters:
    ...
    maximum_distance - [in]
      ...
      then maximum_distance is ignored.
    sub_domain0 - [in]
      If not nullptr, the search is confined to "s" parameters
      in the intersection of the sub_domain0 interval and (0,1).
    sub_domain1 - [in]
      If not nullptr, the search is confined to "t" parameters
  ...
  */
  ...
  /*
  Description:
    Given a set of bicubic bezier surfaces that share a vertex P,
    and form a complete ring around V, 
    with common edges having identical control points,
    adjust the control points at and adjacent to V so that 
    the normals at V are all N, and the angle between normals along
    common edges is minimized.
  Parameters:
    BezArray - [in, out] The input bezier surfaces.  The do not have to be in order
                    around the common vertex. They will be adjusted in place.
    P - [in]        The desired location of the common vertex
    N - [in]        The desired normal at the vertex.
    pMaxAngleDeviation - [out] max angle between normals along a shared edge (degrees).
  Returns:
    true if successful.
    false if the input beziers do not meet the criterion or
          if there are other failures.
  Note: 
    Since only the control points at or adjacent to the vertex are changed,
    The normals and locations along the edges opposite the vertex do not change.
    */
  static bool MatchTangency(ON_SimpleArray<ON_BezierSurface*>& BezArray,
    ON_3dPoint P,
    ON_3dVector N,
