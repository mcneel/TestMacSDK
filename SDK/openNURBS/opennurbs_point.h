...
public:
  ...
  */
  ...
  /*
  Description:
    Calculate point mass properties of the point array.
  Parameters:
    mp - [out]
    bFirstMoments - [in] true to calculate first moments,
                         mass, and point mass centroid.
    bSecondMoments - [in] true to calculate point mass second moments.
    bProductMoments - [in] true to calculate point mass product moments.
  Returns:
    True if successful.
  */
  bool PointMassProperties(
...
#if defined(OPENNURBS_PLUS)
class ON_CLASS ON_PointPairing
{
public:
...
public:
  ...
  /// </summary>
  /// <param name="point_list">
  /// Specifies which list to set.
  /// </param>
  ...
  /// </returns>
  bool SetPoints(
    ON_PointPairing::List point_list,
  ...
  /// </summary>
  /// <param name="point_list">
  /// Specifies which point list(s) will be set.
  /// </param>
  /// <param name="maximum_chord_variance">
  /// There are two ways to sample points from the curve, equal parameter and equal arc length.
  /// Equal parameter sampling samples the curve at equally spaced parameter values.
  /// Equal arc length sampling samples the curve at equally spaced arc length values.
  /// In general, equal parameter sampling is faster while equal arc length sampling produces
  /// more uniformly distributed points.  The maximum_chord_variance parameter controls
  /// what type of sampling is used.
  /// If maximum_chord_variance = 1.0, then equal arc length sampling is unconditionally used.
  /// If maximum_chord_variance &gt; 1.0, then equal arc length sampling is used
  /// if the ratio of (longest chord) / (shortest chord) calculated from the equal parameter points
  /// is &gt; maximum_chord_variance. 
  /// Otherwise, equal parameter sampling is used unconditionally. 
  /// Suggestion:
  /// If you want equal parameter sampling, plass ON_DBL_QNAN.
  /// If you want equal arc length sampling, plass 1.0.
  /// If you want to strike a balance between speed and a uniform sampling, pass 4.0.
  /// </param>
  /// <param name="curve">
  /// The curve is evaluated to create the point list.
  /// </param>
  /// <param name="curve_subdomain">
  /// If you want to restrict the evaluation to subset of the curve,
  /// pass an increasing interval that is contained in curve.Domain().
  /// If you want to evaluate of the entire curve, pass
  /// curve.Domain(), ON_Interval::EmptyInterval, or ON_Interval::Nan.
  /// </param>
  /// <param name="bPeriodicPoints"></param>  
  /// <param name="point_count"></param>  
  /// <returns>
  /// True if the points were set.
  /// </returns>
  bool SetPointsFromCurve(
    ON_PointPairing::List point_list,
  ...
  /// </param>
  /// <param name="surface">
  /// The surface is evaluated to create the point list.
  /// </param>
  /// <param name="surface_subdomain0">
  /// If you want to restrict the evaluation to subset of the surface,
  /// pass an increasing interval that is contained in surface.Domain(0).
  /// If you want to evaluate of the entire surface, pass
  /// surface.Domain(0), ON_Interval::EmptyInterval, or ON_Interval::Nan.
  /// </param>
  /// <param name="surface_subdomain1">
  /// If you want to restrict the evaluation to subset of the surface,
  /// pass an increasing interval that is contained in surface.Domain(1).
  /// If you want to evaluate of the entire surface, pass
  /// surface.Domain(1), ON_Interval::EmptyInterval, or ON_Interval::Nan.
  /// </param>
  /// <param name="bPeriodicPoints0"></param>  
  /// <param name="bPeriodicPoints1"></param>  
  /// <param name="point_count0">
  /// Grid count in the first parameter domain direction. The total number of pionts will be is point_count0 * point_count1.
  /// </param>  
  /// <param name="point_count1">
  /// Grid count in the second domain direction. The total number of pionts will be is point_count0 * point_count1.
  /// </param>  
  /// <returns>
  /// True if the points were set.
  /// </returns>
  bool SetPointsFromSurface(
    ON_PointPairing::List point_list,
  ...
  const ON_3dPoint FirstPoint(
    ON_PointPairing::List point_list
  ) const;

  const ON_3dPoint LastPoint(
    ON_PointPairing::List point_list
  ) const;

  /// <returns>
  /// If both point lists have the same number of points, then that number is returned.
  /// Otherwise 0 is returned.
  /// </returns>
  const unsigned PairCount() const;

  /// <summary>
  /// If A[] and B[] are the point lists, then
  /// the point in B[] closest to A[k] is B[Pairing(k).j]
  /// and
  /// the point in A[] closest to B[k] is A[Pairing(k).i].
  /// </summary>
  /// <param name="point_index">
  /// 0 &lt;= point_index &lt; PairCount()
  /// </param>
  /// <returns>
  /// If the A and B point lists have the same number of points, then
  /// the pairing indices described above are returned.
  /// Otherwise ON_2udex::Unset is returned.
  /// </returns>
  const ON_2udex Pairing(
    unsigned point_index
  ) const;

  /// <summary>
  /// The pairing deviation is 1/2 
  /// the sum of the squared distances from each point in A[] to the nearest point in B[],
  /// plus 
  /// the sum of the squared distances from each point in B[] to the nearest point in A[].
  /// The 1/2 is applied to simplify derivative caluculations involving distance squared.
  /// </summary>
  /// <param name="bNormalized">
  /// When bNormalized is true, the sum described above is divided by PairCount() and the
  /// returned value is the average of the squared distances in the sums. The normalized value is
  /// useful when studying the effect of the number of points on the PairingDeviation value
  /// and when you want to work with smaller values on large point sets.
  /// </param>
  /// <returns>
  /// If A[] and B[] have the same number of points, then the pairing deviation between A[] and B[]
  /// is returned. Otherwise ON_DBL_QNAN is returned.
  /// </returns>
  double PairingDeviation(bool bNormalized) const;

  /// <summary>
  /// The chamfer distance is 
  /// the sum of the distances from each point in A[] to the nearest point in B[],
  /// plus 
  /// the sum of the distances from each point in B[] to the nearest point in A[].
  /// </summary>
  /// <param name="bNormalized">
  /// When bNormalized is true, the sum described above is divided by 2*PairCount() and the
  /// returned value is the average of the distances in the sums. The normalized value is
  /// useful when studying the effect the number of points on the ChamferDistance and
  /// when you want to work with smaller values on large point sets.
  /// </param>
  /// <returns>
  /// If A[] and B[] have the same number of points, then the chamfer distance between A[] and B[]
  /// is returned. Otherwise ON_DBL_QNAN is returned.
  /// </returns>
  double ChamferDistance(bool bNormalized) const;

  /// <summary>
  /// This is useful if you need to know when the values returned by
  /// PairingDeviation() and ChamferDistance() are cached or need 
  /// to be evaluated from scratch.
  /// </summary>
  /// <returns>
  /// True if the PairingDeviation() and ChamferDistance() evaluations
  /// are cached.
  /// </returns>
  bool EvaluationsCached() const;

  /// <summary>
  /// Get the indices of the points in A[] and B[] that have the maximum separation.
  /// The i component of the returned ON_2dex is always an index into A[].
  /// The j component of the returned ON_2dex is always an index into B[].
  /// </summary>
  /// <param name="list">
  /// list = ON_PointPairing::List::A or ON_PointPairing::List::B.
  /// If list = A, then the maximum separation from a point in A[] to its closest point is B[] is returned.
  /// If list = B, then the maximum separation from a point in B[] to its closest point is A[] is returned.
  /// Note that these are typially different pairs.
  /// </param>
  /// <returns>
  ...
  /// <summary>
  /// Get a line connecting the points in A[] and B[] that have the maximum separation.
  /// The from point is always an A[] point.
  /// The to point is always a B[] point.
  /// </summary>
  /// <param name="list">
  /// list = ON_PointPairing::List::A or ON_PointPairing::List::B.
  /// If list = A, then the maximum separation from a point in A[] to its closest point is B[] is returned.
  /// If list = B, then the maximum separation from a point in B[] to its closest point is A[] is returned.
  /// Note that these are typially different lines.
  /// </param>
  /// <returns>
  ...
  /// A or B. 
  /// If source and destination identify the same list, nothing is changed.
  /// </param>
  /// <returns>
  /// True if the copy was successful.
  /// False if the input parameters were invalid.
  /// </returns>
...
private:
  ON_SimpleArray<ON_3dPoint> m_points[2];
  bool m_bPeriodicPoints = false;

  /// <summary>
  /// m_B_points[m_pairing[k].i] is the B[] point closest to m_A_points[k];
  /// m_A_points[m_pairing[k].j] is the A[] point closest to m_B_points[k];
  /// </summary>
  mutable ON_SimpleArray<ON_2udex> m_pairing;

  /// <summary>
  /// The pairing deviation is 1/2 the sum of distances squared.
  /// </summary>
  mutable double m_pairing_deviation = ON_DBL_QNAN;

  /// <summary>
  /// The chamfer distance is the sum of distances.
  /// </summary>
  mutable double m_chamfer_distance = ON_DBL_QNAN;

  /// <summary>
  /// The distance from m_points[0][m_max_B_separation.j] to m_points[1][m_max_A_separation.i]
  /// is the maximum distance from any point in A[] to the closest point in B[].
  /// </summary>
  mutable ON_2udex m_max_A_separation_pair = ON_2udex::Unset;

  /// <summary>
  /// The distance from m_points[0][m_max_B_separation.j] to m_points[1][m_max_A_separation.i]
  /// is the maximum distance from any point in A[] to the closest point in B[].
  /// </summary>
  mutable ON_2udex m_max_B_separation_pair = ON_2udex::Unset;

private:
  static unsigned Internal_ListIndex(ON_PointPairing::List list);
  /// <summary>
  /// Finds the point in the point list P[] that is closest to Q and returns the index of that point and the square of the distance from Q to that point.
  /// </summary>
  /// <param name="Q">
  /// Test point. 
  /// If P[] is the list of target points, then Q is a point on the fit curve.
  /// If P[] is the list of list points obtained by evaluating the fit curve at the target paramters, then Q is a target point.
  /// </param>
  /// <param name="P_count">
  /// Number of points in P[]
  /// </param>
  /// <param name="P">
  /// Sequence of points. This is either the list of target points or the list points obtained by evaluating the fit curve at the target paramters.
  /// </param>
  /// <param name="Pdex">
  /// The input value should be the value returned for P[] and Q the last time this function was called. 
  /// If that value is not known, pass the index of Q in its list.
  /// The output value is the index of the closest P[] to Q.
  /// </param>
  /// <returns>
  /// The square of the distance from Q to the P[] that is closest to Q.
  /// </returns>
  static const double Internal_ClosestPoint(
    ON_3dPoint Q,
  ...
  void Internal_ClearPairing(bool bEmptyPairingArray) const;

  static double Internal_Encode2udexAsDouble(ON_2udex pair);

  static const ON_2udex Internal_Decode2udexInDouble(double y);

  bool Internal_UpdatePairs() const;  
};
#endif
