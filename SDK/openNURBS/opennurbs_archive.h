...
Parameters:
  pObject - [in]
Returns:
  If successful, an integer N > 0 which indicates the object
  was saved in a file named debug_file_XXXXX.3dm, where 
  XXXXX is a %05d format of N. 
  The file is in the directory named ON_DebugWriteObjectDirectory().
See Also:
...
/*
Description:
  ON_DebugWriteObjects is a debugging utility that can be called
  from C++ or from a debugger's evaluate expression window to 
  dump objects for future inspection.
Parameters:
	object_list_count - [in] explicit object list count 
...
/*
Description:
  ON_DebugWriteObjectArray is a debugging utility that can be called
  from C++ or from a debugger's evaluate expression window to 
  dump objects for future inspection.
Parameters:
	object_list - [in]  object list
...
*/
ON_DECL
int ON_DebugWriteObjectArray( 
  const ON_SimpleArray< const class ON_Object*>& object_list
);

/*
Description:
  ON_DebugWritePoint is a debugging utility that can be called
  from C++ or from a debugger's evaluate expression window to 
  dump 3d points for future inspection.
Parameters:
  p3dPoint - [in]
Returns:
  If successful, an integer N > 0 which indicates the object
...
/*
Description:
  ON_DebugWritePoints is a debugging utility that can be called
  from C++ or from a debugger's evaluate expression window to
  dump an array of points as a 3d ON_PointCloud for future inspection.
Parameters:
  pPoint - [in]
  dim - [in] dimension of points must be 2 or 3
  is_rat -[in]  true if points are homogeneous
...
*/
ON_DECL
int ON_DebugWritePoints(const  double* pPoint, int dim, bool is_rat, int count);
