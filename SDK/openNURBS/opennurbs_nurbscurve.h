...
public:
  ...
  */
...
#if defined(OPENNURBS_PLUS)
  // Description:
  //   If in is increasing, call Trim(in).
  //   If in is not increasing - 
  //     If this is closed, make the subcurve given by the subdomain,
  //       [in[0], this->Domain()[1]] U [this->domain, in[1]].
  //       The resulting domain will be [in[1], in[0]+this->Domain.Length()]
  //     Otherwise fail.
  // Parameters:
  //   in - [in] a proper subdomain of the curve.
  // Returns:
  //   true if successful.
  bool TrimClosedCurve(const ON_Interval& in);
	...
	/*	 
  Description:
    Remove knots from a curve and adjust the remaining control points to maintain curve 
		position as closely as possible.
  Parameters:
		i0, i1 - [in] ( Order()-2 < i0 < i1 <= CVCount()-1 
			The knots from Knot(i0) through Knot(i1-1) will be removed.
		tolerance -[in] ( >= 0.0) 
			If tolerance > 0, then the knots are removed only if the resulting
			TL_NurbsCurve will match the input TL_NurbsCurve within the specified tolerance.
			If tolerance == 0.0, the knots are removed and the geometry of the
			resulting NURBS matches that of the input NURBS as closely as possible.
  Returns:
    true if successful.  
		*/
	bool RemoveKnots(	int i0,					// Knot(i0), ..., Knot(i1-1) will be removed 
										int i1,   
										double  tolerance=0.0 // >=0.0. use 0.0 for no tolerance checking 
										);
#endif
...
public:
  ...
  */
  ...
  /// <summary>
  /// Modify the control points so the curve
  /// interpolates points[] at the Greville abcissa
  /// of the current knot vector.
  /// The curve's dimension rational, order, control point count,
  /// properties must be set. 
  /// The curve must have dimension &lt;= 3. 
  /// If this curve is rational, the control point weights must be set.
  /// </summary>
  /// <param name="points">
  /// If bPeriodic is false, points[] must have length CVCount().
  /// If bPeriodic is true, points[] must have length CVCount() - Degree().
  /// </param>
  /// <param name="bPeriodic">
  /// True if the resulting curve should be periodic.
  /// </param>
  /// <returns>
  /// True if successful.
  /// </returns>
  bool GrevilleInterpolate(
  ...
  /// of the current knot vector.
  /// The curve's dimension, rational, order, control point count,
  /// properties must be set. If this curve is rational, 
  /// the control point weights must be set.
  /// </summary>
  /// <param name="points">
  /// points to interpolate. These have dimension &gt;= Dimension().
  /// </param>
  /// <param name="point_count">
  /// NUmber of points. If bPeriodic is false, point_count must have length CVCount().
  /// If bPeriodic is true, point_count must have length CVCount() - Degree().
  /// </param>
  /// <param name="point_stride">
  /// Number of doubles to skip between points. (point_stride &gt;= Dimension()).
  /// </param>
  /// <param name="bPeriodic">
  /// True if the resulting curve should be periodic.
  /// </param>
  ...
  /// </param>
  /// <param name="point_count">
  /// Number of points. This must be identical to cv1-cv0.
  /// </param>
  /// <param name="point_stride">
  /// Number of doubles to skip between points. (point_stride &gt;= this Dimension()).
  /// </param>
  /// <param name="bPeriodic">
  /// True if the resulting curve should be periodic.
  /// </param>  
  /// <param name="cv0">
  /// 0 &lt;= cv0 &lt; cv1 &lt;= CVCount().
  /// The free control points have indices i satisfying cv0 &lt;= i &lt; cv1.
  /// </param>
  /// <param name="cv1">
  /// 0 &lt;= cv0 &lt; cv1 &lt;= CVCount().
  /// The free control points have indices i satisfying cv0 &lt;= i &lt; cv1.
  ...
  /// </summary>
  /// <param name="points">
  /// points to interpolate. points[] must have length (cv1-cv0).
  /// </param>
  /// <param name="bPeriodic">
  /// True if the resulting curve should be periodic.
  /// </param>  
  ...
  //   true if successful
  // Remarks:
  //   Degree 1 NURBS curves cannot be unclamped.
  // See Also:
  //   SetUnclampedTagForExperts
  bool UnclampEndForExperts(
    int end
  );
#endif
