...
#include "opennurbs_plus_crashtest.h"
...
int MyGlobalCrashTestFunction( int crash_type, ON_TextLog& textlog )
{
  ...
  return CrashTestHelper(crash_type,textlog); // static function defined in opennurbs_plus_crashtest.h
}
...
{
  int* ptr;
  CrashTestHelper_GetNullIntPrt(&ptr); // sets ptr = nullptr
  textlog.Print("nullptr ptr = %08x\n",ptr);
  *stack_ptr = *ptr;    // dereferences nullptr pointer and crashes
  textlog.Print("*ptr = %d\n",*ptr);
  return crash_type;
}
...
int CrashTestHelper_CallNullFuncPtr( ON_TextLog& textlog, int crash_type, int* stack_ptr )
{
  ...
  CrashTestHelper_GetNullFuncPtr(&fptr); // sets ptr = nullptr
  textlog.Print("nullptr function f = %08x\n",fptr);
  *stack_ptr = fptr(crash_type); // dereferences nullptr function pointer and crashes
...
{
  CRASHTEST__FUNCTION__POINTER__ fptr;
  CrashTestHelper_GetBogusFuncPtr(&fptr); // sets ptr = nullptr
...
static
bool CrashTestHelper_AllocationFailure(  ON_TextLog& textlog, size_t sz )
{
  void* p = onmalloc(sz);
  textlog.Print("onmalloc(%llu) = %p\n",sz,p);
  onfree(p);  
  return (nullptr != p);
}

#pragma ON_PRAGMA_WARNING_PUSH
#pragma ON_PRAGMA_WARNING_DISABLE_MSC(4717)
static 
bool CrashTestHelper_StackOverflow(ON_TextLog& textlog, int depth=0)
{
  CrashTestHelper_StackOverflow(textlog, depth + 1);
  puts("trick compiler into not reusing stack frame.");
  return true;
}
...
Parameters:
  crash_type - [in]
    ...
    2: dereference nullptr function pointer
...
int CrashTestHelper( int crash_type, ON_TextLog& textlog )
{
  ...
  switch( crash_type )
  {
  case 0: // dereference nullptr pointer
  ...
  case 2: // call nullptr function pointer
  ...
  case 12:
    if ( CrashTestHelper_AllocationFailure( textlog, 0xABCDABCDABCDABCDllu ) )
    {
      textlog.Print("Attempt to allocate 0xABCDABCDABCDABCD did not crash.\n");
    }
    break;

  case 13:
    if (CrashTestHelper_StackOverflow(textlog, 0))
    {
      textlog.Print("Attempt to crash with stack overflow failed");
    }
