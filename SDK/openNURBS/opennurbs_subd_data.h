  ...
  */
  ...
  Parameters:
    bEnableApproximatePatch - [in]
      ...
      been performed and a minimum of 2 subdivisions have been performed.
...
public:
  /// <summary>
  /// Sets ON_SubDComponentBase.m_symmmetry_set_next = 0 for every component in the heap.
  /// </summary>
  void ClearSymmetrySets();

  unsigned ClearSymmetrySet(ON_SubDComponentPtr cptr);

  unsigned SymmetrySetCount(unsigned symmetry_motif_count, ON_SubDComponentPtr cptr) const;

  const ON_SubDComponentPtr SymmetrySetPrimaryMotif(unsigned symmetry_motif_count, ON_SubDComponentPtr cptr) const;

  const ON_SubDComponentPtr SymmetrySetNext(ON_SubDComponentPtr cptr) const;

  unsigned GetSymmetrySet(
    unsigned symmetry_motif_count, 
  ...
  bool SetSymmetrySet(
    ...
    const ON__UINT_PTR* component_symmetry_set,
...
private:
  static bool Internal_SetSymmetrySetFail(const ON_SimpleArray<ON_SubDComponentPtr>& component_symmetry_set, unsigned int clear_count);
#endif
...
public:
  ////////////////////////////////////////////////////////////
  ///
  /// SubDimple symmetry interface
  /// 
  const class ON_Symmetry& Symmetry() const;
  void ClearSymmetry();
  void SetSymmetry(const class ON_Symmetry & symmetry, bool bSetSymmetricObjectContentSerialNumber);
#endif
...
{
public:
  ON_SubDSurfaceNurbsFragment m_patch_fragment = ON_SubDSurfaceNurbsFragment::Empty;
  unsigned int m_display_density = 0;
  unsigned int m_patch_count = 0;
  ON__UINT_PTR m_fragment_callback_context = 0;
  bool (*m_fragment_callback_function)(ON__UINT_PTR, const class ON_SubDSurfaceNurbsFragment*) = nullptr;
  bool m_bCallbackResult = false;
  
  bool Send4x4Patch(
    unsigned int display_density,
    const class ON_SubDFaceRegion& face_region,
    ON_SubDSurfaceNurbsFragment::BispanAccuracy bispan_type
    );
  
  bool Send5x5Patch(
    unsigned int display_density,
    const class ON_SubDFaceRegion& face_region,
    const ON_SubDSurfaceNurbsFragment::BispanAccuracy bispan_type[4]
    );
};
...
public:
  ...
  */
  bool SetDestinationToMeshFragment(
    unsigned int mesh_density,
    class ON_SubDMeshFragment& mesh_fragment
    );
...
private:
 ...
 /*
  Description:
    Get values of derivatives of cubic uniform B-spline basis functions.
  Parameters:
    t - [in] 0.0 <= t <= 1.0
      Evaluation parameter
    b - [out]
...
private:
  ...
  ON_SubD_FixedSizeHeap m_fshx[6];
